...
底部 Dock 在探索态使用筛选、观测记录、全球和设置四个等权槽位；进入全球后同一
基座扩展成带目录遥测的轨道控制台，第三槽切换为时间。捕获状态仍由 `SkyChromeState`
提高中央主动作的优先级。`TimeDial` 只在全局轨道明确打开时间面板时从控制台上方展开，
非 LIVE 的“返回此刻”和全球视场复位收进控制台遥测行。
