...
9. `OrbitalBootView` 只从业务准备链读取聚合后的 `BootPreparationState.isReady`。启动轨道必须来自
   ...
   必须批量绘制。启动页与全局页共享镜头构建、投影、地球材质和点云分层，启动页可独立指定
   上海纬度与相机侧倾、白色点对比，不能改变全局默认镜头。点群是不可变示意分布，近赤道带
   可为视觉可读性夸张，但不能冒充星链真实分布或真实目录快照。3.2 秒编舞只允许
   使用可解析积分的连续速度曲线完成静置、扫过、刹停和定格，拖尾必须取真实历史时刻的相位。
   慢加载只能从定格连续恢复为低速巡航，不能扩容、循环电影或重置地球与卫星相位。
