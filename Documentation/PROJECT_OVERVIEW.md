...
- 灵动岛左翼以精巧信号灯配合状态文字：未捕获与降级为红、感知和对焦为黄、识别成功及锁定为绿；
  释放时逐渐收暗。右翼 AZ / EL 固定标识，整数数值以最高 6Hz 上下滚动，抑制动效时直接更新。
...
设置从底部四槽控制基座向上生长为固定大面板。根页展示显示偏好、确认捕获、系统状态、手册、
...
- 筛选、记录、设置和仪器状态统一使用 56pt 顶栏：左返回、右标题。根页向下返回天空，
  可顶栏下拉或外部点击收回；子页保留左缘返回上一层。手册、隐私和深度档案保持沉浸式阅读。
