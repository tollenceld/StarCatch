...
- SwiftUI 第一帧立即建立与主天空连续的固定星场和预设轨道地球；16,507 条真实目录继续在后台准备，
  不阻塞窗口建立。
- 首次与回访使用同一段约 3.2 秒的纯享地球电影：4,600 个确定性解析轨道目标中最多 24 个带
  方向清晰的时间轨迹；画面先静置建立，再完成一次主要卫星约 44–72° 的扫过和约 20° 的地球
  转动，随后连续刹停并留出约 1.2 秒构图定格。底部低透明度 `STARCATCH` 在收势后才出现，
  不与主体争夺注意力。
- 启动镜头在定格时以上海（31.2304°N、121.4737°E）为中心，并施加 -14° 相机侧倾；全局页的
  赤道水平默认镜头不变。两者共享镜头构建、投影、球体半径、卫星前后景分层、表面材质、经纬网、
  点阵大陆和前景轮廓绘制。约 52 KB 的陆地点阵资源只在后台映射解码一次；尚未准备好时使用同一份编译期
  基础海岸线，Canvas 每帧不做文件 IO。
- 启动场景不读取轨道目录、真实星表、位置或 SatelliteKit，不执行 SGP4、网络或 Shader；它不
  显示返回、时间轴、当前位置、加载模块、状态文字或按钮。
- 点云是明确的视觉示意，不冒充真实卫星快照：74% 使用 43°/53° 的密集星座壳层，18% 使用
  为可读性夸张的近赤道带，8% 使用高倾角轨道；显示半径与动画时间也非物理比例。核对本地
  2026-09-09 目录时，11,078 个 STARLINK 名称目标主要位于约 43°/53° 倾角，而非赤道环。
  [Starlink 官方壳层说明](https://space-safety.starlink.com/docs/space-safety-articles/constellation_altitudes/)
  支持该倾角区别；不得将示意赤道带描述为星链实际分布。点云沿用全局页批量绘制，启动页仅提高白色点的对比。
- 运动使用五次缓动及其解析积分，地球与卫星共用一次手势时基，两端速度与加速度连续；
  普通卫星的公共相位每帧只计算一次，不按帧累积位置。
- 真实准备在 3.2 秒内完成时等待电影定格后交叉淡入主天空；准备较慢时从静止连续恢复为低速
  巡航，完成后立即进入，不重新黑场、不跳变速度或重置轨道相位。
...
- 常驻层只保留准星、低强度方位参考，以及底部同尺寸的筛选、全局轨道和设置入口；
...
筛选从四槽控制基座的等宽入口打开独立全屏页面。顶栏下固定显示当前筛选摘要与结果数量；
...
- 当前观察者位置使用分层呼吸坐标点，默认可见范围使用双层细刻度球面边界；两者保持在卫星层
  上方，说明标签只在点按定位符后短暂出现，默认画面不遮挡大陆。
...
- 默认镜头以观察者经度为中央经线，未授权定位时对准上海经度；地轴的屏幕投影保持竖直、赤道
  ...
  因此地表沿赤道左右展开，惯性恒星保持静止。触摸旋转、缩放或拨动时间会立即暂停，相关惯性
  结束 2 秒后从原角度续转。
- 双击复位默认朝向、0.82× 缩放和自动旋转相位；系统或 App 开启 Reduce Motion 时自动旋转关闭。
- 顶部使用位于灵动岛下方的整条全球模式栏：左侧返回天空，中部标明全球轨道，右侧显示 LIVE
  或时间偏移；本地天空专属的 AZ / EL 不进入全球页。
...
设置从底部四槽控制基座打开独立全屏页面。根页展示显示偏好、确认捕获、系统状态、手册、
隐私、系统设置入口和版本信息，不再重复展示观测记录摘要。系统状态由设置流自己的
`NavigationStack` 推进，包含：
...
- 应用内提供完整隐私说明。
- 提供公开隐私政策和支持链接。
- 筛选、观测记录、设置和仪器状态统一使用 56pt 顶栏：左侧返回上一层，右侧显示当前页面标题；
  根页和记录页支持左缘返回天空。手册、隐私和深度任务档案继续使用原有沉浸式阅读形态。
- 三个工具页共享暖黑实底、分组表面与顶部高光边界，覆盖期间不显示主天空底部控制基座。
...
- iOS 26 及以上：四槽控制基座、全球轨道控制台与功能胶囊使用统一 Liquid Glass 容器；
  iOS 17–25 使用同布局的超薄材质、深黑覆层和统一描边。全屏工具页保持标准暖黑暗色材质。
...
- 启动层只运行固定星场、4,600 个解析轨道点、最多 24×8 段短拖尾和共享地球 Canvas；点阵地理
  数据在后台只解码一次，30fps 路径不执行目录遍历、图片解码、文件 IO、SGP4 或 Shader。超过
  3.2 秒后卫星进入低速巡航，地球保持与全局页相同的慢速展示旋转；Reduce Motion 下直接呈现
  无旋转、无拖尾的稳定终态。
- 目录预建 ID、类别和星座家族索引。
