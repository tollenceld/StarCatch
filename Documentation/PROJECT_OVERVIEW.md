...
筛选从四槽控制基座的等宽入口打开中/大两档系统 Sheet。默认中档允许继续操作背景天空，
顶部先显示“全部轨道、可读档案、载人与科学、地球观察”四个常用镜片，下方完整保留显示范围、
任务、机构和星座选项；所有选择实时生效，无需确认按钮，拖动抓手、下滑或点按内嵌“完成”
即可收起。Sheet 使用暖黑基底、二级分组表面与顶部高光边界，和天空保持同一色相但层级清晰。
...
- 当前观察者位置与默认可见范围的球面标记保持在卫星层上方；说明标签只在点按定位符后短暂出现，
  默认画面不遮挡大陆。
...
- 底部为两层全球轨道控制台：上层集中显示已显示/目录计数、LIVE/时间状态与按需复位，下层为
  筛选、观测记录、时间、设置四个等宽入口。目录计数不在 30fps Canvas 中绘制。
- 旋转或缩放后由控制台状态行提供复位；双击仍可同时复位旋转与缩放。
...
时间入口只存在于全局星图 Dock，主天空不常驻时间轴。点按后，读数与刻度作为悬浮面板出现在
Dock 上方；这样时间旅行与全局空间语境强绑定，也不让刻度长期遮挡三维地球。
...
设置从底部四槽控制基座打开中/大两档模态系统 Sheet。根页优先展示显示偏好、确认捕获和观测摘要；
系统状态、观测记录与指定记录详情由 Sheet 内的 `NavigationStack` 推进，包含：
...
- 应用内提供完整隐私说明。
- 提供公开隐私政策和支持链接。
- Sheet 内二级页面使用标准顶部左侧返回与右侧上下文操作；手册、隐私和深度任务档案继续使用
  原有沉浸式阅读形态。
- 筛选与设置根页共享暖黑基底、分组表面、顶部高光边界和内嵌文字标题栏，避免与后方天空融为
  一体；观测记录同时保留为本地与全球控制基座的直接入口。
...
- iOS 26 及以上：四槽控制基座、全球轨道控制台与功能胶囊使用统一 Liquid Glass 容器；
  iOS 17–25 使用同布局的超薄材质、深黑覆层和统一描边。Sheet 内容保持标准暖黑暗色材质。
...
| 状态 | 唯一所有者 |
| --- | --- |
| 启动、全屏阅读与互斥 Sheet | `RootView` / `AppSheetDestination` |
...
完整测试集 121/121 通过，包含四槽配置、Chrome 优先级、Sheet 路由、Overlay 互斥、材质回退、真实惯性时基与
