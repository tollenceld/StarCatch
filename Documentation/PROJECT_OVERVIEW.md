...
筛选从底部 Dock 的等尺寸入口打开中/大两档系统 Sheet。默认中档允许继续操作背景天空，
顶部先显示“全部轨道、可读档案、载人与科学、地球观察”四个常用镜片，下方完整保留显示范围、
任务、机构和星座选项；所有选择实时生效，无需确认按钮，拖动抓手或下滑即可收起。
...
设置从底部 Dock 打开中/大两档模态系统 Sheet。根页优先展示显示偏好、确认捕获和观测摘要；
...
- 工程生成：XcodeGen。
- 最低部署：iOS 17。
- iOS 26 及以上：天空 Dock 与功能胶囊使用 Liquid Glass；iOS 17–25 使用同布局的超薄材质、
  深黑覆层和统一描边。Sheet 内容保持标准暗色材质。
...
完整测试集 118/118 通过，包含 Chrome 优先级、Sheet 路由、Overlay 互斥、真实惯性时基与
