...
底栏“全球”是地心三维轨道场的直接入口。主天空缩到最广视场后继续缩小，可随手势退离地表，
松手时决定完成进入或返回天空；全球内部捏合不会触发返回。该页面与主天空共享同一份
ECI 轨道传播结果，不是静态示意图。
...
- 单指水平和垂直旋转。
- 拖动结束后的惯性收束。
- 主天空在 0.52×–4× 内缩放；继续缩小到原始倍率 0.32× 对应完整全球。松手进度达到 50%
  完成进入，否则回到 0.52×，中途可反向撤回；捕获或锁定期间仅保留局部缩放。
- 按钮入口与捏合使用同一共享相机，完整自动旅程约 1.05 秒。启动与返回在中段短暂引导到当地
  地平线上方 12°，让弧面表达尺度，抵达后准确接回设备指向。返回保留全球当前旋转与缩放起点，
  历史先回 LIVE 再推进。Reduce Motion 不显示飞行预览，松手后约 0.16 秒淡化。
