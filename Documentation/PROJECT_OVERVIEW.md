...
- SwiftUI 第一帧立即建立与主天空连续的低密度星场；16,395 条轨道目录在后台准备，不阻塞窗口建立。
- `STARCATCH` 标题固定不移动，窄幅焦平面依次唤醒字母；分段校准轨、光学边界和目录／轨道／姿态三项模块状态共同表达真实准备进度。
- 状态点、系统名称与阶段文字使用同一基线的自适应布局，中英文不依赖固定宽度占位。
- 首次启动页由用户主动进入，不用固定时长抢走阅读内容。
- 若点击时目录仍在准备，应用会记住意图并在完成后立即进入观测，不会淡回黑屏。
- 回访用户不重复观看介绍；准备完成后标题与模块状态克制退场，同一片星场直接承接主天空。
- 手册可以从设置中重新打开。
...
- 稳定阅读：底部摘要显示“轨道在列”等可信目录状态、名称、当前洞察、微型过境弧或
  轨道指纹，以及距离、高度、速度；不再复制通用任务故事。
- 深入档案：先用任务角色、用途短句和可信身份说明“它是什么、用来做什么”，再通过
  “观测／任务／数据”三种阅读视图组织当前天空、轨道指纹、任务资料、历程和结构化来源。
...
- 当前方位、仰角、距离趋势、星下点和完整过境窗口。
- 周期、倾角、偏心率、近地点与远地点组成的轨道指纹。
...
星座成员共享经过核实的系列背景，但当前身份、轨道指纹、批次与实时洞察保持独立。
...
摘要和深入档案共用 `SatelliteMotionSignature`：过境目标使用真实过境进度，LEO/MEO/HEO
按周期、倾角、离心率和方向呈现，GEO 保持站位稳定。30fps 时间轴只插值 Canvas 几何，
不会触发 SGP4、文件读取或目录遍历；系统或 App 开启减少动态效果、页面隐藏或进入后台时停止。
...
- 同时输出升起、最高点、最大仰角与落下组成的完整窗口。
- 结果按对象、粗粒度观察者坐标和五分钟时间桶缓存；目标切换会取消旧任务。
- 计算位于 utility 后台任务，摘要卡不会等待 24 小时扫描才出现。
...
- 启动层只运行少量确定性文字、细线和星点 Canvas 动效，不执行目录遍历、图片解码、
  SGP4、模糊或 Shader；Reduce Motion 下直接呈现稳定终态。
- 目录预建 ID、类别和星座家族索引。
