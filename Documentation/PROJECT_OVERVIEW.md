...
- 首次与回访使用同一段约 3.2 秒的纯享地球电影：4,600 个确定性解析轨道目标中最多 24 个带
  方向清晰的时间轨迹；画面先静置建立，再完成一次卫星扫过和约 5° 的地球转动，随后连续刹停并
  留出约 1.2 秒构图定格。底部低透明度 `STARCATCH` 在收势后才出现，不与主体争夺注意力。
- 启动地球与全局页共享镜头姿态、球体半径、卫星前后景分层、表面材质、经纬网、点阵大陆和
  前景轮廓绘制。约 52 KB 的陆地点阵资源只在后台映射解码一次；尚未准备好时使用同一份编译期
...
完整测试集 121/121 通过，包含四槽配置、Chrome 优先级、工具页路由、Overlay 互斥、材质回退、真实惯性时基与
