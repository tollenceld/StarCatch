...
| 应用 Swift 源文件 | 41 个 |
| Swift 测试文件 | 2 个 |
| Swift 代码与测试总行数 | 约 12,037 行 |
| 单元测试方法 | 60 个 |
| `StarCatch` 源码与资源目录 | 约 19 MB |
| `catalog.json` | 约 11.7 MiB |
| 当前工作区 | 约 34 MB，不含 `.git` 与 DerivedData |
| 最低系统版本 | iOS 17.0 |
| 设备方向 | iPhone、竖屏、全屏 |
| 应用版本 | 1.0（Build 1） |
...
- SwiftUI 第一帧立即显示品牌、用途说明和明确的“进入观测”按钮；16,243 条轨道目录在后台准备，不阻塞窗口建立。
- 启动信息与按钮在约 0.2 秒内分两拍干脆显现，不使用与主天空相似的星点元素。
- 首次启动页由用户主动进入，不用固定时长抢走阅读内容。
- 若点击时目录仍在准备，应用会记住意图并在完成后立即进入观测，不会淡回黑屏。
- 回访用户不重复观看介绍；准备窗口仅显示极简品牌文字，完成后直接进入天空。
- 手册可以从设置中重新打开。
- Reduce Motion 开启时，启动和手册转场自动缩短。
...
- 第一眼：名称、运行状态、当前距离和一句任务说明。
- 稳定阅读：方位、高度、速度、轨道类型，以及 LIVE 状态下的下一次升起、落下或 GEO 常驻提示；过去或未来时刻显示时间坐标。
- 深入档案：COSPAR / NORAD 标识、任务机构、发射历史、工程意义、关键节点、来源和完整事实字段。

准星刚刚感应目标时只显示第一层；即时模式稳定驻留后开放遥测，确认捕获模式则在真正锁定后开放。由此避免普通面板在第一帧退化成参数表。

### 3.6 深度任务档案

3,832 个非大型星座对象按 NORAD 各自拥有一份可编辑的离线 Markdown；
...
完整即时信息出现时，独立的“深入档案”入口会在画面下方中部浮现，不再跟随空间面板移动。单体卫星读取自己的档案；大型星座成员进入项目共享档案。深度页面拥有统一的顶部返回导航，不会遮挡主天空设置入口。
...
左上角全局入口打开地心三维轨道场。该页面与主天空共享同一份 ECI 轨道传播结果，不是静态示意图。
...
- 全局星图实时拖尾：约 10 fps 采样，最长约 9 秒，每个对象最多 90 个点。
...
```mermaid
flowchart TD
    A[系统 Launch Screen] --> A1[立即可见的品牌与用途说明]
    A1 --> A2[后台准备离线轨道目录]
    A2 --> B{是否首次使用}
    B -->|是| C[极简产品署名]
    C -->|点击进入观测| E[主天空]
...
| --- | --- |
| Schema | 2 |
| 快照时刻 | 2026-07-26 08:30:10 UTC |
| 生成时刻 | 2026-07-26 08:30:10 UTC |
...
16,243 个对象不能按传统 SwiftUI 子视图逐个建立。当前工程采用：

- JSON 只在目录加载时解析一次。
- 目录读取、JSON 解码、SatelliteKit 对象和筛选索引在首帧之后于后台构建。
- 启动层只有静态文字、细线与按钮，不运行持续动画、图片解码、模糊或 Shader。
- 目录预建 ID、类别和星座家族索引。
...
```bash
python3 Scripts/update_catalog.py --output StarCatch/Resources/catalog.json
```
