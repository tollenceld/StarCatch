...
- `STARCATCH` 标题固定不移动，通过锐利遮罩和收束注册线一次性完成字形显影；目录／轨道解算／观测模型三项账本状态直接对应真实准备节点。
- 启动动画不使用模糊扫光、移动圆点或循环时间轴；数据延迟时停在稳定画面，已完成的节点不会重播。
...
- 全局星图只为当前锁定对象显示空间拖尾：约 10 fps 采样，最长约 9 秒、最多 90 个点；
  普通对象只显示实时位置，避免数千条轨迹把地球变成线框笼。
