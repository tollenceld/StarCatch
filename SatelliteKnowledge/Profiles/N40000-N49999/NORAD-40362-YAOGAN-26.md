---
type: satellite-profile
schema: 1
norad: 40362
catalog_name: YAOGAN-26
eyebrow: EARTH OBSERVATION ARCHIVE
organization: PUBLICLY REGISTERED REMOTE-SENSING SERIES
program: YAOGAN-26
review_status: generated
---

# YAOGAN-26

## 摘要

当前目标是 2014-088A / N40362：约 93.0 分钟绕地一周，沿97.1° 的近极轨道运行。 公开目录将它识别为遥感系列；未公开的具体载荷和用途不在这里推测。

## 正文

### 可以确认的任务边界

公开目录将它识别为遥感系列；未公开的具体载荷和用途不在这里推测。 当前目标是 2014-088A / N40362：约 93.0 分钟绕地一周，沿97.1° 的近极轨道运行。

### 这条轨道的性格

它约 93.0 分钟绕地一周，运行在97.1° 的近极轨道上。平均轨道高度约 420 千米。离心率 0.00012，轨道非常接近圆形。这些数值描述轨道元素历元附近的结构，并不是永久不变的轨道铭牌。

### 与它一同进入目录的对象

当前离线快照只保留了 2014-088 这次发射的这一条在轨记录，它是其中的 A 号分件。 国际编号关联的是发射事件，不等同于任务归属；这里仅陈述当前离线快照能够交叉确认的对象关系。

## 时间线

- 2014 | 国际编号记录的发射年份；当前目标以 2014-088A 进入公开目录
- 2026-07-25 | 随 App 打包的 GP/OMM 平均轨道元素历元

## 事实

- NORAD | 40362
- COSPAR | 2014-088A
- 分类 | 地球与大气观测对象
- 类型 | 科学或技术任务
- 轨道 | LEO
- 状态 | ACTIVE
- 周期 | 约 93.0 分钟
- 倾角 | 97.07°
- 离心率 | 0.000121
- 估算近 / 远地点 | 419 / 421 KM

## 来源

- UNOOSA · Space object registration records · https://www.unoosa.org/oosa/en/spaceobjectregister/index.html
- CelesTrak · GP/OMM 轨道目录 · https://celestrak.org/NORAD/documentation/gp-data-formats.php
- NASA Earth Observatory · 轨道、倾角与离心率参考 · https://science.nasa.gov/earth/earth-observatory/catalog-of-earth-satellite-orbits/
